        ...
        P: AsRef<Path>,
    {
        ...
        Self {
            queries: QueryVcs::new(root.as_ref(), VcsPreset::Hg, config, ProcessRunner)
                .with_fragment_layout(fragment_directory.as_ref(), section_directories),
        }
    }
}
...
impl<R: Runner> QueryVcs<R> {
    ...
    fn with_fragment_layout(mut self, directory: &Path, sections: Vec<PathBuf>) -> Self {
        self.fragment_directory = Some(directory.to_path_buf());
...
fn possible_fragment_path(path: &Path, directory: &Path, sections: &[PathBuf]) -> bool {
